...
def test_sequential_native_preparation_uses_distinct_video_bytes_and_hashes(tmp_path):
    first, second = tmp_path / "first.mp4", tmp_path / "second.mp4"
    first.write_bytes(b"first video bytes")
    second.write_bytes(b"different second video bytes")

    with native_av.prepare_native_av(str(first), "caption", include_audio=False) as (first_content, first_meta):
        first_data = first_content[0]["input_video"]["data"]
    with native_av.prepare_native_av(str(second), "caption", include_audio=False) as (second_content, second_meta):
        second_data = second_content[0]["input_video"]["data"]

    assert first_data != second_data
    assert first_meta["video_sha256"] != second_meta["video_sha256"]
    assert "exactly one clip" in second_content[-1]["text"]
...
def test_streaming_native_request_disables_prompt_cache(monkeypatch):
    posted = {}
    ...
    monkeypatch.setattr(app.requests, "post", fake_post)
    assert list(app._stream_chat_completion([], "model", cache_prompt=False)) == []
