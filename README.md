...
Native preparation diagnostics are emitted at INFO level by CaptionHelper's application logger, so they are visible in the Linux terminal even when Flask debug mode is off. During a batch, the active-file display shows preparation/readiness status, and each completed or failed item adds a compact native AV line to the browser terminal containing the first 12 SHA-256 characters plus audio status (and size/duration for successful items). Full provenance remains in the caption metadata sidecar.
