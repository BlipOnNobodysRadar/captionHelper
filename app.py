...
def _h3_format_correction_content(content:list, errors:list[str])->list:
    """Reuse encoded media while replacing only the textual instruction."""
    correction = (
        "Your prior answer failed format validation: " + " ".join(errors)
        + " Return the caption again with exactly these three field labels in order: "
          "integrated_multimodal_description:, overall_soundscape:, non_diegetic_music:. "
          "Start directly with integrated_multimodal_description:. No Markdown or commentary."
    )
    corrected = [part for part in content if part.get("type") != "text"]
    corrected.append({"type": "text", "text": correction})
    return corrected
...
def _log_h3_validation(label:str, caption:str, caption_meta:dict)->None:
    """Emit diagnostic prose to the server terminal, not the compact browser log."""
    validation = caption_meta.get("validation") or {}
    if caption_meta.get("retried"):
        app.logger.warning(
            "H3 format retry for %s. Initial errors=%s\n--- initial model output ---\n%s\n--- retry model output ---\n%s\n--- end H3 diagnostic ---",
            label,
            (caption_meta.get("initial_validation") or {}).get("errors", []),
            caption_meta.get("initial_model_output") or "",
            caption,
        )
    if not validation.get("valid"):
        app.logger.error("H3 validation failed for %s after retry=%s: %s", label, bool(caption_meta.get("retried")), validation.get("errors", []))
...
def _stream_chat_completion(messages:list, model:str, max_output_tokens:int=DEFAULT_MAX_OUTPUT_TOKENS, cache_prompt:bool=True):
    payload = {
        "model": model or DEFAULT_MODEL,
        "messages": messages,
        "temperature": 0.2,
...
def chat_caption_stream():
    def generate():
        ...
        try:
            ...
            for event_type, token in _stream_chat_completion(
                messages,
                prepared["model"],
                prepared["max_output_tokens"],
                cache_prompt=prepared["video_input_mode"] != "native_av",
            ):
